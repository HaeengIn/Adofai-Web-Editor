export class EditorSettingsController {
  ...
  initEditorSettingsUI() {
    ...
    const savedOffset = Number(
      app.readEditorPreference("adofai-editor-offset-ms", "0"),
    );
  ...
  openLevelLoadHelp() {
    const hidden =
      this.app.readEditorPreference("adofai-editor-hide-load-help-v1", "0") ===
      "1";
    const levelInput = document.getElementById("level-file-input");
  ...
  async loadSongFromProjectUrl(projectPath) {
    const songFilename = String(
      this.app.doc?.settings?.songFilename ?? "",
    ).trim();
  ...
  async selectSongFile(
    ...
    const loaded = await this.app.song.init(
      this.app.hitSound.ctx,
      this.app.localSongObjectUrl,
    );
